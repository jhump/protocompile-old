...
func (l *protoLex) Lex(lval *protoSymType) int {
	...
	for {
		...
		if c >= '0' && c <= '9' {
			...
			l.readNumber(true, true)
			numstr := l.input.getMark()
...
func (l *protoLex) readNumber(allowDot bool, allowExp bool) {
	for {
